...
func TestPreparePassThroughJSONBodyInjectsOpenAIFastTier(t *testing.T) {
	...
	body, closer, err := PreparePassThroughJSONBody(storage, dto.ChannelOtherSettings{ForceOpenAIFast: true})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Equal(t, int64(len(out)), body.Size())
	assertJSONEqual(t, `{"model":"gpt-5","input":"hello","service_tier":"priority"}`, string(out))
	...
	require.NoError(t, err)
	assertJSONEqual(t, string(out), string(replayed))
}

func TestPreparePassThroughJSONBodyPreservesReplayWithoutRewrite(t *testing.T) {
	input := []byte(`{"model":"gpt-5","service_tier":"flex"}`)
	storage, err := common2.CreateBodyStorage(input)
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, storage.Close()) })
	...
	require.Equal(t, int64(len(input)), body.Size())
